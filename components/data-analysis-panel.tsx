...
export function DataAnalysisPanel() {
  const { uploadedFiles, simulateRealTimeData } = useUnifiedStore()
  const [mounted, setMounted] = useState(false)
  
  // Mock data sources based on uploaded files
  const dataSources: DataSource[] = uploadedFiles.map(file => ({
    id: file.id,
    name: file.name,
    type: file.type.includes('csv') ? 'sales_records' : 
          file.type.includes('database') ? 'customer_database' : 
          file.type.includes('email') ? 'email_campaigns' : 'invoice',
    status: file.status === 'completed' ? 'completed' : 
            file.status === 'analyzing' ? 'processing' : 'error',
    progress: file.analysisProgress || 0,
    records_count: file.recordsProcessed || Math.floor(Math.random() * 10000) + 1000,
    last_processed: file.status === 'completed' ? file.uploadTimestamp : null
  }))
