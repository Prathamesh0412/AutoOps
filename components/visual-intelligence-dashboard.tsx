...
const MiniBarChart = ({ data, color = "#10b981" }: { data: number[], color?: string }) => {
  const max = Math.max(...data)
...
export function VisualIntelligenceDashboard() {
  ...
  const computedMetrics = useMemo(() => {
    const executedActions = actions.filter((a: any) => a.status === 'executed')
    const pendingActions = actions.filter((a: any) => a.status === 'pending')
    
    // Time saved calculation (based on action types and impact)
    const timeSavedPerAction: { [key: string]: number } = {
      'email_campaign': 2, // 2 hours saved
      'inventory_order': 4, // 4 hours saved
      'lead_assignment': 1.5 // 1.5 hours saved
    }
    
    const totalTimeSaved = executedActions.reduce((total: number, action: any) => {
      return total + (timeSavedPerAction[action.type] || 1)
    }, 0)
    ...
    const workflowSuccessRate = workflows.filter((w: any) => w.is_active).length / (workflows.length || 1) * 100
    
    const systemHealth = (avgConfidence * 0.4 + actionSuccessRate * 0.3 + workflowSuccessRate * 0.3)
    
    // Churn rate calculation
    const churnRate = customers.length > 0 
      ? customers.filter((c: any) => c.churn_risk > 0.7).length / customers.length * 100 
      : 0

    // Revenue impact calculation (based on customer LTV and actions)
    const totalRevenueImpact = customers.reduce((total: number, customer: any) => {
      return total + (customer.ltv || 0) * 0.1 // Assume 10% LTV impact from AI actions
    }, 0)

    // Inventory health calculation
    const inventoryHealth = products.length > 0 
      ? Math.max(0, 100 - (products.filter((p: any) => (p.stock || 0) < 10).length / products.length * 100))
      : 100
  ...
  }, [actions, predictions, workflows, customers, products])
