...
class VirtualDatabase {
  private storageKey = 'autoops-virtual-db'
  ...
  async addDataSource(dataSource: Omit<DataSource, 'id' | 'created_at'>): Promise<DataSource> {
  ...
  async addPrediction(prediction: Omit<Prediction, 'id' | 'created_at'>): Promise<Prediction> {
    const data = this.getData()
  ...
  async updateAction(id: string, updates: Partial<Action>): Promise<Action> {
    const data = this.getData()
    const index = data.actions.findIndex((a: Action) => a.id === id)
    if (index !== -1) {
  ...
  async executeWorkflow(id: string): Promise<Workflow> {
    const workflow = await this.updateWorkflow(id, {
      ...
      total_executions: (await this.getWorkflows()).find(w => w.id === id)!.total_executions + 1
    })
    return workflow
  }
